...
class LinkedInCompanyCollector(BaseCollector):
    ...
    async def collect(self, context: dict) -> CollectorResult:
        ...
        # Нужен либо прямой URL либо имя/домен для поиска
        if not company_name and not domain:
            return make_not_applicable_result(self.source_name, "No company_name or domain")

        # Пробуем через Parser API (структурированный результат)
        linkedin_url = self._build_linkedin_url(company_name or domain)
        try:
            ...
            self.logger.debug(f"Parser API failed: {exc}, trying proxy scrape")

        # Fallback: Proxy + BS4
        try:
            html = await self.proxy.get_html(
                linkedin_url, residential=True, render_js=True
            )
            data = self._parse_html(html)
            if not data:
                return make_failed_result(self.source_name, linkedin_url, "Could not parse LinkedIn page")
            return CollectorResult(
                source_name=self.source_name,
                status="partial",
                data=data,
            ...
            return make_failed_result(self.source_name, linkedin_url, str(exc))
